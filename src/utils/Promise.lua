...
function Promise.any(promises)
	return Promise._all(debug.traceback(nil, 2), promises, 1):andThen(
		function(values)
			return values[1]
		end
	)
...
function Promise.race(promises)
	assert(
		type(promises) == "table",
		string.format(ERROR_NON_LIST, "Promise.race")
	)
...
function Promise.each(list, predicate)
	...
	assert(
		...
		string.format(ERROR_NON_FUNCTION, "Promise.each")
	)
	...
	return Promise._new(
		...
		function(resolve, reject, onCancel)
			...
			for index, value in ipairs(preprocessedList) do
				...
				local predicatePromise = Promise.resolve(
...
function Promise.is(object)
	...
	elseif
		...
		and type(rawget(rawget(objectMetatable, "__index"), "andThen"))
			== "function"
	then
				...
				if connection == nil then -- first is nil when connection is nil
					...
					connection = Promise._timeEvent:Connect(function()
						...
						while first ~= nil and first.endTime < threadStart do
							...
							current.resolve(
								Promise._getTime() - current.startTime
							)
						end
					end)
...
function Promise.retry(callback, times, ...)
	...
	assert(
		type(times) == "number",
		"Parameter #2 to Promise.retry must be a number"
	)
...
function Promise.fromEvent(event, predicate)
	...
	return Promise._new(
		debug.traceback(nil, 2),
		function(resolve, _reject, onCancel)
			local connection
